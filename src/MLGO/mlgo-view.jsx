...
// In handleReturnToLGU function, replace the notification section with:
...
// Fetch LGU answers with role-based filtering - MODIFIED to handle verified assessments
useEffect(() => {
...
useEffect(() => {
  if (!auth.currentUser || !selectedYear) return; // Add selectedYear dependency
  ...
  const fetchIndicators = async () => {
    try {
      ...
      if (usersSnapshot.exists()) {
        ...
        if (adminUid) {
          // Use selectedYear from state instead of hardcoding "2026"
          const indicatorsRef = ref(
            db,
            `financial/${adminUid}/${selectedYear}/financial-administration-and-sustainability/assessment`
          );
      ...
      >
        <div style={{ 
          ...
          <div className={style.assessmentTabs}>
            <button className={style.activeTab}>
  ...
  >
    {loading ? (
      ...
      <>
        {lguAnswers.length > 0 ? (
          (userRole === "user" 
            ? lguAnswers.filter(lgu => lgu.lguName.toLowerCase() === userMunicipality.toLowerCase())
            : lguAnswers
          ).map((lgu) => (
            <div key={lgu.id}>
            ...
            return indicatorAttachments.length > 0 && (
              <div style={{
                ...
                <p style={{ textAlign: "center", marginTop: "20px" }}>
                  No indicators available for this assessment.
                </p>
              )}
            </div>
          ))
