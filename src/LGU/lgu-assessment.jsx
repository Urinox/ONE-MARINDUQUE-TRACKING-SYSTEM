...
const downloadAttachment = (attachment) => {
  ...
  useEffect(() => {
    ...
    if (navigationState && navigationState.fromNotification) {
      ...
      // Show an alert or notification based on the type
      if (navigationState.isReturned && navigationState.mlgoRemarks) {
        setTimeout(() => {
          const remarksText = typeof navigationState.mlgoRemarks === 'object' 
            ? Object.values(navigationState.mlgoRemarks).join(", ")
            : navigationState.mlgoRemarks;
          alert(`Assessment returned with remarks: ${remarksText}`);
        }, 500);
      } else if (navigationState.isVerified) {
        setTimeout(() => {
          alert("Assessment has been verified!");
        }, 500);
      } else if (navigationState.isForwarded) {
        setTimeout(() => {
          alert("Assessment has been forwarded to PO!");
        }, 500);
      }
